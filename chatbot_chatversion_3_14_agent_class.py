...
def _ensure_sas_session() -> bool:
    """Ensure there is an active SAS session in session_state."""
    try:
        if st.session_state.get('sas') is None:
            try:
...
def tool_execute_question(question: str) -> Dict[str, Any]:
    """Execute a natural-language sub-question via the existing single-step pipeline.

    Returns a dict with keys: question, query, df, summary, error, log.
    """
    result: Dict[str, Any] = {
        'question': question,
        'query': '',
    ...
    # Validate
    val = validate_question_llm(question)
    if val.get('error'):
        result['error'] = val['error']
        return result

    q = val.get('validated_question', question)
    result['question'] = q

    # Generate SQL
    gen = generate_query_single_step(q)
    sql = (gen.get('query') or '').strip()
    result['query'] = sql
    if not sql:
        result['error'] = 'Failed to generate SQL for the sub-question.'
        return result

    # Ensure SAS and run
    if not _ensure_sas_session():
        result['error'] = t('sas_connect_fail')
        return result

    df, err, log = run_sas_query(sql)
    result['df'] = df
    result['error'] = err
    result['log'] = log or ''
    ...
    if df is not None and err is None:
        try:
            result['summary'] = _summarize_df_for_email(df)
        except Exception:
            try:
                rows, cols = df.shape
                head_csv = df.head(3).to_csv(index=False)
                result['summary'] = f"Rows={rows}, Cols={cols}\nPreview:\n{head_csv}"
            except Exception:
                result['summary'] = 'Summary unavailable.'
    return result


class MultiStepTrainAgent:

    def __init__(self, max_steps: int = 3):
        self.max_steps = max_steps

    def _decision(self, user_question: str, scratchpad: List[Dict[str, Any]]) -> str:
        """Ask the LLM what to do next.

        Returns a controller string starting with either:
        - "ACTION: <sub-question>" to call the tool; or
        - "FINAL: <answer>" to finish.
        """
        history_lines = []
        for i, step in enumerate(scratchpad, 1):
            act = step.get('action', '') or ''
            obs_err = step.get('observation', {}).get('error')
            obs_sum = step.get('observation', {}).get('summary')
            obs_shape = ''
            try:
                df = step.get('observation', {}).get('df')
                if df is not None:
                    obs_shape = f" (rows={len(df)}, cols={len(df.columns)})"
            except Exception:
                pass
            line = f"Step {i} Action: {act}\nObservation{obs_shape}: "
            if obs_err:
                line += f"ERROR: {obs_err}"
            elif obs_sum:
                # Keep it compact
                preview = obs_sum
                if len(preview) > 800:
        ...
        prompt = f"""
You are a senior data analyst. You can call a tool that, given a natural-language train question, validates it, generates PROC SQL, executes it in SAS, and returns a small textual summary of the results.

Goal: Answer the user's question precisely. If needed, decompose into up to {self.max_steps} tool calls.

Rules:
- When you want the tool to run, respond with: ACTION: <single, concrete sub-question to run>
- When you are ready to answer the user, respond with: FINAL: <concise answer>
- Prefer minimal steps. If you already can answer, use FINAL.
- Use insights from prior observations. Do not repeat identical queries.
- The tool only has data for 2023 and 2024.

User question:
{user_question}

Prior steps and observations:
{history_text}

Your next response must start with either ACTION: or FINAL:
"""
        return call_gemini(prompt).strip()

    def run(self, user_question: str) -> Dict[str, Any]:
        """Run the multi-step loop and return a transcript and final answer."""
        scratchpad: List[Dict[str, Any]] = []
        final_answer = ''

        for _ in range(self.max_steps):
            decision = self._decision(user_question, scratchpad)
            if decision.upper().startswith('FINAL:'):
                final_answer = decision[len('FINAL:'):].strip()
                break
            if decision.upper().startswith('ACTION:'):
                subq = decision[len('ACTION:'):].strip()
                obs = tool_execute_question(subq)
                scratchpad.append({'action': subq, 'observation': obs})
                # If the tool returns a hard error, provide it as context and continue one more step
                continue
            # Fallback: if the model returns something else, try one best-effort sub-query
            obs = tool_execute_question(user_question)
            scratchpad.append({'action': user_question, 'observation': obs})

        if not final_answer:
            # Ask the model to synthesize an answer from the scratchpad
            synth_lines = []
            for i, s in enumerate(scratchpad, 1):
                ...
                sql = obs.get('query')
                if err:
                    synth_lines.append(f"Step {i} error: {err}")
                if sql:
                    synth_lines.append(f"Step {i} SQL used:\n{sql}")
                if summary:
                    synth_lines.append(f"Step {i} summary:\n{summary}")
            synth_text = "\n\n".join(synth_lines) or "No data retrieved."
            prompt = f"""
Using the following intermediate results, write a concise, direct answer to the user's question. If data is missing or errors occurred, state limitations clearly. Keep it under 6 sentences.
...
def should_use_agent(user_question: str) -> bool:
    """Auto-decide if the query likely needs multi-step reasoning.

    Strategy:
    - Fast heuristics for obvious multi-part phrasing.
    - Fallback to a tiny LLM classifier that returns AGENT or SIMPLE.
    """
    try:
        ...
        heuristic_tokens = [
            'then', 'first', 'next', 'after that', 'based on that', 'based on those',
            'for those', 'for them', 'for each', 'per each', 'followed by',
            'using that', 'using those', 'for the top', 'and then'
        ]
        if any(tok in q for tok in heuristic_tokens):
            return True

        # Patterns like: "top 5 ... then ..."
        try:
            if re.search(r"top\s+\d+.*\bthen\b", q):
                return True
        except Exception:
            pass

        # Multiple interrogatives or complex conjunctions can hint chaining
        if q.count('?') >= 2:
            return True

        # Fallback to LLM decision for borderline cases
        prompt = f"""
Decide if the user's question requires multiple sequential data retrieval steps (e.g., find a set, then compute metrics for that set), or a single direct retrieval.
Answer with EXACTLY one token: AGENT or SIMPLE.

Question:
{user_question}
"""
        ans = call_gemini(prompt).strip().upper()
        return ans.startswith('AGENT')
    except Exception:
        return False
...
def main() -> None:
    ...
    if user_message:
        ...
        # Automatic decision: multi-step agent vs simple single-step flow
        if should_use_agent(user_message):
            agent = MultiStepTrainAgent(max_steps=3)
            with st.spinner('Analyzing and running sub-queries...'):
                out = agent.run(user_message)

            # Render steps
            for i, step in enumerate(out.get('steps', []), 1):
                act = step.get('action', '')
                obs = step.get('observation', {})
                st.session_state['messages'].append({'role': 'assistant', 'content': f"Step {i} action: {act}"})
                if obs.get('query'):
                    st.session_state['messages'].append({'role': 'assistant', 'content': {'type': 'code', 'data': obs['query']}})
                if obs.get('error'):
                    st.session_state['messages'].append({'role': 'assistant', 'content': f"Error: {obs['error']}"})
                if obs.get('log') and 'ERROR:' in (obs.get('log') or '').upper():
                    st.session_state['messages'].append({'role': 'assistant', 'content': {'type': 'log', 'data': obs.get('log')}})
                if obs.get('df') is not None:
                    st.session_state['messages'].append({'role': 'assistant', 'content': {'type': 'dataframe', 'data': obs['df']}})
                    # Append each agent step result to sidebar history
                    try:
                        st.session_state['query_history'].append({
                            'question': (act or '').strip() or '(agent step)',
                            'data': obs['df'].copy()
                        })
                    except Exception:
                        # Fallback without copy if needed
                        try:
                            st.session_state['query_history'].append({
                                'question': (act or '').strip() or '(agent step)',
                                'data': obs['df']
            ...
            final_text = out.get('final', '').strip()
            if final_text:
